...
const APP_NAME: &str = "Radium";
const LEGACY_APP_NAME: &str = "JewWare";
const TRAY_ID: &str = "radium-status";
...
const CURRENT_ACCOUNT_KEY_NAMESPACE: &str = "radium-accounts-v1";
const LEGACY_ACCOUNT_KEY_NAMESPACE: &str = "jewware-accounts-v1";
...
fn build_tray_menu(app: &AppHandle, state: &AppStateHandle) -> Result<Menu<tauri::Wry>, String> {
    ...
    let header = MenuItem::with_id(app, TRAY_MENU_HEADER, "Radium Direct", false, None::<&str>)
        .map_err(|e| e.to_string())?;
    ...
    let open_item = MenuItem::with_id(app, TRAY_MENU_OPEN, "Open Radium", true, None::<&str>)
        .map_err(|e| e.to_string())?;
...
fn join_website(app: AppHandle) -> AppResult {
    let website_path = get_website_path(&app)
        .ok_or_else(|| "Radium website files were not found".to_string())?;
    Command::new("open")
        .arg(website_path)
        .stdout(Stdio::null())
        .stderr(Stdio::null())
...
fn export_accounts(state: State<'_, AppStateHandle>) -> AppResult {
    ...
    let Some(file_path) = rfd::FileDialog::new()
        .set_file_name("radium-accounts.json")
        .save_file() else {
...
fn main() {
    let state = AppStateHandle::new().expect("failed to initialize Radium paths");
